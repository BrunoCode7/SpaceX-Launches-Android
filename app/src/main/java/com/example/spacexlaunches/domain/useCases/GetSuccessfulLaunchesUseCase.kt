...
class GetSuccessfulLaunchesUseCase @Inject constructor(private val repository: LaunchesRepository){

    operator fun invoke():Flow<Resource<List<LaunchItem>>> = flow {
        try {
            emit(Resource.Loading)
            val data = repository.getLaunches()
            val filteredData = data.filter { launchItem -> launchItem.success == true }
            emit(Resource.Data(filteredData))
